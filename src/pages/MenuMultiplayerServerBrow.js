...
const MenuMultiplayerServerBrow = () => {
  ...
  const fetchAllSettings = async () => {
    try {
      const response = await fetch(
        "https://fog-battlefield-backend.vercel.app/api/settings/all"
      );
      const settingsData = await response.json();
      setAllSettings(settingsData);
    } catch (error) {
      console.error("Error fetching all settings data:", error);
    }
  };

  // Assuming you need to access individual settings like basicSettings, advancedSettings, and rulesData
  // You can now access them from the 'allSettings' object

  // Example usage:
  const basicSettings = allSettings ? allSettings.basicSettings : null;
  const advancedSettings = allSettings ? allSettings.advancedSettings : null;
  const rulesData = allSettings ? allSettings.rulesData : null;

  // Render UI components using basicSettings, advancedSettings, and rulesData
